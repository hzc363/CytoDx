#' Build the two stage statistical model (tssm) using gradient boosting
#' machine.
#'
#' A function that builds the two stage statistical model (tssm) using
...
#' # build the two stage statistical model
#' fit =tssm.gbm(x=x,y=y,xSample=xSample,family="binomial")
#' @importFrom gbm gbm.fit
#' @importFrom stats predict
#' @export
tssm.gbm = function(x,y,xSample,family,type1="response",type2="response",...){
  ...
  preds = predict(fit,newx = x,type=type1,n.trees = fit$n.trees)
  ...
  return(list("train.Data1"=train.Data,"model1"=fit,"train.Data2"=as.data.frame(SP.train),
              "model2"=fit2,"family"=family,"method"="gbm","type1"=type1,"type2"=type2))
}
